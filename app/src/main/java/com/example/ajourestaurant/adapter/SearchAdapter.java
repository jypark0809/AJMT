...
public class SearchAdapter extends RecyclerView.Adapter<SearchAdapter.ViewHolder> {
    ...
    public SearchAdapter(ArrayList<Restaurant> restaurantsList, Context context) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        SearchAdapter.ViewHolder viewHolder = new SearchAdapter.ViewHolder(itemView);
        return viewHolder;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int position) {
        ...
        if(closeTime>86400) { // 마감시간이 24:00보다 늦게 끝난다면
            if(closeTime <= curTime && curTime <= openTime) {
                isOpen = false;
            } else {
                isOpen = true;
            }
        ...
        if(isOpen == true) {
            viewHolder.iv_isOpen.setImageResource(R.drawable.img_open);
            viewHolder.tv_open_time.setTextColor(0xAA00DA6F);
            viewHolder.tv_close_time.setTextColor(0xAA00DA6F);
            viewHolder.tv_tilde.setTextColor(0xAA00DA6F);
        } else {
            viewHolder.iv_isOpen.setImageResource(R.drawable.img_close);
        }
        viewHolder.tv_name.setText(restaurantsList.get(position).getName());
        viewHolder.tv_type.setText(restaurantsList.get(position).getType());
        viewHolder.tv_menu.setText(restaurantsList.get(position).getMenu());

        DecimalFormat formatter = new DecimalFormat("###,###");
        int decimalForamt_price = restaurantsList.get(position).getPrice();
        viewHolder.tv_price.setText(String.valueOf(formatter.format(decimalForamt_price))+"원");
        viewHolder.tv_rating.setText(String.valueOf(restaurantsList.get(position).getRating()));
        viewHolder.tv_open_time.setText(restaurantsList.get(position).getOpen());
        viewHolder.tv_close_time.setText(restaurantsList.get(position).getClose());
    }

    @Override
    public int getItemCount() {
        return (restaurantsList != null ? restaurantsList.size() : 0 );
    }
